...
namespace LandscapeBuilder
{
    public class LBEditorHelper
    {
            ...
            if (annotationUtilityType != null)
            {
                ...
                foreach (object annotation in (IEnumerable)annotations)
                {
                    ...
                    if (classIdField != null && scriptClassField != null)
                    {
                        ...
                        {
                            // bool addToMostRecentChanged was added in U2019.1
        ...
        public static void SaveEditorLayout(string layoutName)
        {
            ...
            Type windowLayout = LBIntegration.GetClassTypeFromFullName("UnityEditor.WindowLayout, UnityEditor.CoreModule", true);
            #else
        ...
        public static void LoadEditorLayout(string layoutName)
        {
            ...
            if (windowLayout != null && !string.IsNullOrEmpty(layoutName))
            {
                ...
                try
                {
                    // LoadWindowLayout(..) seems to have gained several new parms in 2023.1. TryLoadWindowLayout(..) seems
                    // to do the same as the preview version.
                    #if UNITY_2023_1_OR_NEWER
                    windowLayout.InvokeMember("TryLoadWindowLayout", System.Reflection.BindingFlags.InvokeMethod | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static, null, null, new object[] { filePath, true });
                    #else
            ...
            // We could use the EditorUtility but here is no SaveWindowLayout(..) so ot be consistent use reflection for both.
        ...
        /// Typically would be wrapped in a EditorGUILayout.BeginHorizontal() and EndHorizontal()
        /// </summary>
        /// <param name="indent"></param>
        ...
        /// <param name="maxWidth"></param>
        public static void DrawLabelIndent (float indent, GUIContent guiContent, GUIStyle labelStyle, float maxWidth = 0f)
        {
            DrawLabelIndent(indent);
            if (maxWidth > 0f) { EditorGUILayout.LabelField(guiContent, GUILayout.MaxWidth(maxWidth)); }
            else { EditorGUILayout.LabelField(guiContent); }
        ...
        /// <param name="folderPath"></param>
        /// <param name="fileName"></param>
        public static void DeleteAssetSilently(string folderPath, string fileName)
        {
            string filePath = folderPath + "/" + fileName;
            if (File.Exists(filePath))
            {
        ...
        public static System.Type GetEditorType(string editorTypeName, bool showSuccess, bool showErrors)
        {
            ...
            {
                #if UNITY_2020_2_OR_NEWER
                editorType = System.Type.GetType("UnityEditor." + editorTypeName + ", UnityEditor.CoreModule", true, true);
                #else
        ...
        /// <summary>
        /// Get an asset path from an on-disk full file path from within a Project folder.
        /// </summary>
        /// <param name="filePath"></param>
        /// <returns></returns>
        public static string GetAssetPathFromFilePath (string filePath)
        {
            if (string.IsNullOrEmpty(filePath)) { return string.Empty; }
            else
            {
                // Get the asset folder from the full on-disk file path
                string assetPath = GetAssetFolderFromFilePath(filePath);

                if (string.IsNullOrEmpty(assetPath)) { return string.Empty; }
                else
                {
                    // Add the filename back on the asset folder
