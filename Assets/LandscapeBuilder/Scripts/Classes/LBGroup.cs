...
namespace LandscapeBuilder
{
    ...
    public class LBGroup
    {
                        ...
                        if (numObjPathSeries > 0)
                        {
                            ...
                            for (int osIdx = 0; osIdx < numObjPathSeries; osIdx++)
                            {
                                ...
                                if (lbObjPathSeries != null)
                                {
                                    ...
                                    sb.Append("\t\t\t" + pathSeriesInst + ".includeFixedPositions = " + lbObjPathSeries.includeFixedPositions.ToString().ToLower() + ";" + eol);
        ...
        /// <param name="offset"></param>
        /// <param name="scale"></param>
        /// <param name="isActualHeightsApplied"></param>
        ...
        /// <summary>
        /// When groups are importing from a source landscape, into a destination landscape, typically via a template,
        /// we might want to retain the actual heights of groups (where possible), rather than using the 0.0 to 1.0 height values of the destination landscape.
        /// </summary>
        /// <param name="lbGroup"></param>
        /// <param name="srcLandscapeStart"></param>
        /// <param name="destLandscapeStart"></param>
        /// <param name="srcTerrainHeight"></param>
        /// <param name="destTerrainHeight"></param>
        public static void LandscapeAdjustActualHeight(LBGroup lbGroup, Vector3 srcLandscapeStart, Vector3 destLandscapeStart, float srcTerrainHeight, float destTerrainHeight)
        {
            if (lbGroup != null)
            {
                // Calculate source Actual Heights
                float actualMinHeight = LBLandscape.GetActualHeight(lbGroup.minHeight, srcTerrainHeight, srcLandscapeStart);
                float actualMaxHeight = LBLandscape.GetActualHeight(lbGroup.maxHeight, srcTerrainHeight, srcLandscapeStart);

                if (actualMinHeight < destLandscapeStart.y) { actualMinHeight = destLandscapeStart.y; }
                else if (actualMinHeight > destLandscapeStart.y + destTerrainHeight)
                {
                    actualMinHeight = destLandscapeStart.y + destTerrainHeight;
                    actualMaxHeight = actualMinHeight;
                }

                if (actualMaxHeight < destLandscapeStart.y) { actualMaxHeight = destLandscapeStart.y; }
                else if (actualMaxHeight > destLandscapeStart.y + destTerrainHeight) { actualMaxHeight = destLandscapeStart.y + destTerrainHeight; }
                
                if (actualMaxHeight < actualMinHeight) { actualMaxHeight = actualMinHeight; }

                lbGroup.minHeight = LBLandscape.GetRelativeHeight(actualMinHeight, destTerrainHeight, destLandscapeStart);
                lbGroup.maxHeight = LBLandscape.GetRelativeHeight(actualMaxHeight, destTerrainHeight, destLandscapeStart);

                // Process any members that override the group min/max Heights
                int numGroupMembers = lbGroup.groupMemberList.Count;
                for (int gpmbrIdx = 0; gpmbrIdx < numGroupMembers; gpmbrIdx++)
                {
                    LBGroupMember lbGroupMember = lbGroup.groupMemberList[gpmbrIdx];

                    if (lbGroupMember != null && lbGroupMember.isGroupOverride)
                    {
                        actualMinHeight = LBLandscape.GetActualHeight(lbGroupMember.minHeight, srcTerrainHeight, srcLandscapeStart);
                        actualMaxHeight = LBLandscape.GetActualHeight(lbGroupMember.maxHeight, srcTerrainHeight, srcLandscapeStart);
                        ...
                        lbGroupMember.minHeight = LBLandscape.GetRelativeHeight(actualMinHeight, destTerrainHeight, destLandscapeStart);
                        lbGroupMember.maxHeight = LBLandscape.GetRelativeHeight(actualMaxHeight, destTerrainHeight, destLandscapeStart);
                    }
                }
            }
        }
