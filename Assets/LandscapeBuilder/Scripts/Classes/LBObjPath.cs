...
namespace LandscapeBuilder
{
    ...
    public class LBObjPath : LBPath
    {
        ...
        /// <summary>
        /// Get the index of the next member in the supplied list that does not have any fixed positions assigned to it.
        /// </summary>
        /// <param name="lbObjPrefabList"></param>
        /// <param name="currentIdx"></param>
        /// <param name="isWrapEnabled"></param>
        /// <returns></returns>
        public static int GetNextMemberIndexNoFixedPosition(List<LBObjPrefab> lbObjPrefabList, int currentIdx, bool isWrapEnabled)
        {
            int nextIndex = -1;

            int numItems = lbObjPrefabList == null ? 0 : lbObjPrefabList.Count;

            if (currentIdx < -1 || currentIdx > numItems - 1) { return -1; }
            // If this is the last member and isWrapEnabled is false, there is no next member
            else if (currentIdx == numItems - 1 && !isWrapEnabled) { return -1; }
            else
            {
                ...
                int numIterations = currentIdx < 0 ? numItems : numItems - 1;

                LBObjPrefab _lbObjPrefab = null;
                ...
                for (int i = 0; i < numIterations; i++)
                {
                    ...
                    _lbObjPrefab = lbObjPrefabList[plIdx];
                    if (_lbObjPrefab == null) { continue; }
                    else if (_lbObjPrefab.fixedPositions != null && _lbObjPrefab.fixedPositions.Length > 0) { continue; }
                    else { nextIndex = plIdx; break; }
                }
        ...
        /// <summary>
        /// Has the object path prefab got any fixed positions configured?
        /// </summary>
        /// <param name="lbObjPrefab"></param>
        /// <returns></returns>
        public static bool IsObjPrefabAssignedFixedPositions(LBObjPrefab lbObjPrefab)
        {
            return lbObjPrefab != null && lbObjPrefab.fixedPositions != null && lbObjPrefab.fixedPositions.Length > 0;
        }
