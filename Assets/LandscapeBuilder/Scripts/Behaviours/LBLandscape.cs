// Landscape Builder. Copyright (c) 2016-2023 SCSM Pty Ltd. All rights reserved.
using UnityEngine;
...
public class LBSavedData
{
    ...
    // Added 2.4.2
    public bool isPasteUpdateGroupName = true;
    public bool isPasteUpdateGroups = false;
    ...
    public LBSavedData()
    {
        ...
        isPasteUpdateGroupName = true;
        isPasteUpdateGroups = false;
    }
    #endif
}
#endregion
...
namespace LandscapeBuilder
{
    ...
    public class LBLandscape : MonoBehaviour
    {
        ...
        // Added 2.3.3 (currently not in template)
        [HideInInspector] public bool useCustomBiome = false;
        [HideInInspector] public LBBiome lbBiome = null;
        ...
        /// <summary>
        /// Added 2.4.3 Beta 2a for Unity 2022.2. DetailScatterMode has 2 options
        /// CoverageMode (0) which is the default in U2022.2 but breaks LB.
        ///  - The detail map holds values that represent how much area to cover at each sample, based on the details density
        /// InstanceCountMode (1)
        ///  - The detail map holds values that represent the number of detail instances to render at each sample.
        /// Also, in Unity Project Quality Settings, there is Use Legecy Detail Distribution.
        /// </summary>
        [HideInInspector] public int detailScatterModeInt = 1;

        // Group Options - added v2.4.3
        /// <summary>
        /// When scaling a prefab, use the original prefab scale rather than assume is 1x1x1.
        /// </summary>
        [HideInInspector] public bool isGroupScalePrefabByOriginal = false;
        /// <summary>
        /// The maximum number of seconds to attempt to place a prefab group member in the landscape.
        /// After this time, it gives up and goes to the next group member or group.
        /// </summary>
        [HideInInspector, Range(0.01f, 300f)] public float maxGroupNoPrefabTime = 30f;
        ...
        // Added v2.4.0 Beta 3b
        [HideInInspector] public bool isUndoTexturesDisabled = false;
        [HideInInspector] public bool isUndoGrassDisabled = false;
        [HideInInspector] public bool isUndoTreesDisabled = false;
        ...
        /// <summary>
        /// Get a non-persistent ID for the landscape for comparison during
        /// single sessions.
        /// </summary>
        public int GetNonPersistLandscapeID { get { return GetInstanceID(); } }
        ...
        #region Get and Set Paste Options

        /// <summary>
        /// [INTERNAL ONLY]
        /// Get or retrieve Copy and Paste options from LBSaveFile.dat.
        /// </summary>
        /// <param name="isPasteUpdateGroups"></param>
        /// <param name="isPasteUpdateGroupName"></param>
        public static void GetCopyPasteOptions (ref bool isPasteUpdateGroups, ref bool isPasteUpdateGroupName)
	    {
		    lbSavedData = RetrieveSavedData ();
		    tempLBSavedData = null;
		    if (lbSavedData != null)
            {
                isPasteUpdateGroupName = lbSavedData.isPasteUpdateGroupName;
                isPasteUpdateGroups = lbSavedData.isPasteUpdateGroups;
            }
		    else { Debug.LogWarning("ERROR: Landscape GetCopyPasteOptions could not retrieve saved data"); }
	    }

        /// <summary>
        /// [INTERNAL ONLY]
        /// Save Copy and Paste option to LBSaveFile.dat.
        /// </summary>
        /// <param name="isPasteUpdateGroupName"></param>
        public static void SetCopyPasteOptions (bool isPasteUpdateGroups, bool isPasteUpdateGroupName)
        {
		    ...
		    if (lbSavedData != null)
		    {
			    ...
			    try
			    {
				    binaryFormatter = new BinaryFormatter();
				    fs = File.Open("LandscapeBuilder/LBSaveFile.dat", FileMode.OpenOrCreate);
				    lbSavedData.isPasteUpdateGroups = isPasteUpdateGroups;
				    lbSavedData.isPasteUpdateGroupName = isPasteUpdateGroupName;
				    binaryFormatter.Serialize(fs, lbSavedData);
				    fs.Close();
			    }
			    catch (Exception ex)
			    {
				    Debug.LogWarning("ERROR: Landscape SetCopyPasteOptions Exception - " + ex.Message);
			    }
			    finally
			    {
				    // Cleanup
		    ...
		    else { Debug.LogWarning("ERROR: Landscape SetCopyPasteOptions could retrieve saved data"); }
        }
        ...
        /// <summary>
        /// Import a object path point meta-data from json files given the primary path points file.
        /// </summary>
        /// <param name="lbObjPath"></param>
        /// <param name="folderPath"></param>
        /// <param name="fileName"></param>
        /// <returns></returns>
        public bool ImportPathPointFromJson(LBObjPath lbObjPath, string folderPath, string fileName)
        {
            bool isSuccessful = false;

            if (lbObjPath != null && !string.IsNullOrEmpty(folderPath) && !string.IsNullOrEmpty(fileName))
            {
                try
                {
                    string baseFileName = System.IO.Path.GetFileNameWithoutExtension(fileName);

                    string pathPointsFilePath = System.IO.Path.Combine(folderPath, fileName);
                    string posListFilePath = System.IO.Path.Combine(folderPath, baseFileName + "_PosLMeta.json");
                    string widthListFilePath = System.IO.Path.Combine(folderPath, baseFileName + "_WLMeta.json");
                    string leftEdgeListFilePath = System.IO.Path.Combine(folderPath, baseFileName + "_LELMeta.json");
                    string rightEdgeListFilePath = System.IO.Path.Combine(folderPath, baseFileName + "_RELMeta.json");              

                    // We need at least the PathPoints and Path Position files
                    if (!System.IO.File.Exists(pathPointsFilePath))
                    {
                        #if UNITY_EDITOR
                        Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Could not find file at " + pathPointsFilePath);
                        #endif
                    }
                    else if (!System.IO.File.Exists(posListFilePath))
                    {
                        #if UNITY_EDITOR
                        Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Could not find position metadata file at " + pathPointsFilePath);
                        #endif
                    }
                    else if (lbObjPath.useWidth && !System.IO.File.Exists(widthListFilePath))
                    {
                        #if UNITY_EDITOR
                        Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Could not find width metadata file at " + widthListFilePath);
                        #endif
                    }
                    else if (lbObjPath.useWidth && !System.IO.File.Exists(leftEdgeListFilePath))
                    {
                        #if UNITY_EDITOR
                        Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Could not find width left edge metadata file at " + leftEdgeListFilePath);
                        #endif
                    }
                    else if (lbObjPath.useWidth && !System.IO.File.Exists(rightEdgeListFilePath))
                    {
                        #if UNITY_EDITOR
                        Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Could not find width right edge metadata file at " + rightEdgeListFilePath);
                        #endif
                    }
                    else 
                    {
                        // Read in the mimimum files required
                        string jsonTextPathPoints = System.IO.File.ReadAllText(pathPointsFilePath);
                        string jsonTextPathPositions = System.IO.File.ReadAllText(posListFilePath);

                        if (!string.IsNullOrEmpty(jsonTextPathPoints) && !string.IsNullOrEmpty(jsonTextPathPositions))
                        {
                            // Currently cannot do this because we need a wrapper class for the list
                            //JsonUtility.FromJsonOverwrite(jsonText, pathPointList);

                            List<LBPathPoint> pathPointList = LBUtils.FromJson<LBPathPoint>(jsonTextPathPoints);
                            List<Vector3> positionList = LBUtils.FromJson<Vector3>(jsonTextPathPositions);

                            int numPathPoints = pathPointList == null ? 0 : pathPointList.Count;
                            int numPathPositions = positionList == null ? 0 : positionList.Count;

                            if (numPathPoints < 1)
                            {
                                #if UNITY_EDITOR
                                Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. No path points found in " + pathPointsFilePath);
                                #endif
                            }
                            else if (numPathPoints != numPathPositions)
                            {
                                #if UNITY_EDITOR
                                Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Path (" + numPathPoints + ") and Position (" + numPathPositions + ") files do not contain the same number of points. PLEASE REPORT");
                                #endif
                            }
                            else 
                            {
                                #if UNITY_EDITOR
                                UnityEditor.Undo.RecordObject(this, "Import ObjPath Points");
                                #endif

                                bool isValid = false;

                                List<float> pathWidthList = null;
                                List<Vector3> positionListLeftEdge = null;
                                List<Vector3> positionListRightEdge = null;
                                int numPathWidth = 0, numPathLeftEdge = 0, numPathRightEdge = 0;

                                if (lbObjPath.useWidth)
                                {
                                    string jsonTextPathWidths = System.IO.File.ReadAllText(widthListFilePath);
                                    string jsonTextPathLeftEdge = System.IO.File.ReadAllText(leftEdgeListFilePath);
                                    string jsonTextPathRightEdge = System.IO.File.ReadAllText(rightEdgeListFilePath);

                                    if (!string.IsNullOrEmpty(jsonTextPathWidths) && !string.IsNullOrEmpty(jsonTextPathLeftEdge) && !string.IsNullOrEmpty(jsonTextPathRightEdge))
                                    {
                                        pathWidthList = LBUtils.FromJson<float>(jsonTextPathWidths);
                                        positionListLeftEdge = LBUtils.FromJson<Vector3>(jsonTextPathLeftEdge);
                                        positionListRightEdge = LBUtils.FromJson<Vector3>(jsonTextPathRightEdge);

                                        numPathWidth = pathWidthList == null ? 0 : pathWidthList.Count;
                                        numPathLeftEdge = positionListLeftEdge == null ? 0 : positionListLeftEdge.Count;
                                        numPathRightEdge = positionListRightEdge == null ? 0 : positionListRightEdge.Count;

                                        if (numPathWidth != numPathPoints || numPathWidth != numPathLeftEdge || numPathWidth != numPathRightEdge)
                                        {
                                            #if UNITY_EDITOR
                                            Debug.LogWarning("ERROR: Landscape Builder - Import Object Path Points. Path (" + numPathPoints + "), Width (" + numPathWidth + "), Left (" + numPathLeftEdge + "), and Right (" + numPathRightEdge + ") files do not contain the same number of points. PLEASE REPORT");
                                            #endif
                                        }
                                        else
                                        {
                                ...
                                if (isValid)
                                {
                                    ...
                                    else { lbObjPath.widthList = new List<float>(numPathPoints); }

                                    lbObjPath.pathPointList.AddRange(pathPointList);

                                    // Copy over the data (don't need a deep copy here as they are Vector3 and floats
                                    lbObjPath.positionList.AddRange(positionList);

                                    if (lbObjPath.useWidth)
                                    {
                                        lbObjPath.positionListLeftEdge.AddRange(positionListLeftEdge);
                                        lbObjPath.positionListRightEdge.AddRange(positionListRightEdge);
                                        lbObjPath.widthList.AddRange(pathWidthList);
                                    }
                ...
                {
                    #if UNITY_EDITOR
                    Debug.LogWarning("ERROR: Landscape Builder - could not import Object Path Points from: " + folderPath + " PLEASE REPORT - " + ex.Message);
                    #else
                    // Keep compiler happy
                    if (ex != null) { }
                    #endif
        ...
        /// <summary>
        /// Save the list of LBObjPath points to a json file on disk.
        /// </summary>
        /// <param name="filePath"></param>
        public bool SaveObjPathPointsAsJson (LBObjPath lbObjPath, string filePath)
        {
            bool isSuccessful = false;

            if (lbObjPath != null && lbObjPath.pathPointList != null && lbObjPath.positionList != null && !string.IsNullOrEmpty(filePath))
            {
                try
                {
                    string baseFileName = System.IO.Path.GetFileNameWithoutExtension(filePath);
                    string baseFolderPath = System.IO.Path.GetDirectoryName(filePath);

                    string pathPointsFilePath = filePath;
                    string posListFilePath = System.IO.Path.Combine(baseFolderPath, baseFileName + "_PosLMeta.json");
                    string widthListFilePath = System.IO.Path.Combine(baseFolderPath, baseFileName + "_WLMeta.json");
                    string leftEdgeListFilePath = System.IO.Path.Combine(baseFolderPath, baseFileName + "_LELMeta.json");
                    string rightEdgeListFilePath = System.IO.Path.Combine(baseFolderPath, baseFileName + "_RELMeta.json");

                    // Convert the essential base files
                    string jsonTextPathPoints = LBUtils.ToJson(lbObjPath.pathPointList);
                    string jsonTextPathPositions = LBUtils.ToJson(lbObjPath.positionList);

                    if (!string.IsNullOrEmpty(jsonTextPathPoints) && !string.IsNullOrEmpty(jsonTextPathPositions) && !string.IsNullOrEmpty(filePath))
                    {
                        // Write the essential base files for non-width based Object Paths
                        System.IO.File.WriteAllText(pathPointsFilePath, jsonTextPathPoints);
                        System.IO.File.WriteAllText(posListFilePath, jsonTextPathPositions);

                        isSuccessful = !lbObjPath.useWidth;

                        if (lbObjPath.useWidth)
                        {
                            string jsonTextWidthList = LBUtils.ToJson(lbObjPath.widthList);
                            string jsonTextLeftEdgeList = LBUtils.ToJson(lbObjPath.positionListLeftEdge);
                            string jsonTextRightEdgeList = LBUtils.ToJson(lbObjPath.positionListRightEdge);

                            if (!string.IsNullOrEmpty(jsonTextWidthList) && !string.IsNullOrEmpty(jsonTextLeftEdgeList) && !string.IsNullOrEmpty(jsonTextRightEdgeList))
                            {
                                System.IO.File.WriteAllText(widthListFilePath, jsonTextWidthList);
                                System.IO.File.WriteAllText(leftEdgeListFilePath, jsonTextLeftEdgeList);
                                System.IO.File.WriteAllText(rightEdgeListFilePath, jsonTextRightEdgeList);
                            ...
                            {
                                Debug.LogWarning("ERROR: Landscape Builder - could not export Object Path width-based metadata files for " + transform.name + " PLEASE REPORT");
                            }
                            #endif
                        }
                    }
                    ...
                    {
                        Debug.LogWarning("ERROR: Landscape Builder - could not export Object Path essential path point metadata files for " + transform.name + " PLEASE REPORT");
                    }
                    #endif
                }
                catch (System.Exception ex)
                {
                    isSuccessful = false;
                    #if UNITY_EDITOR
                    Debug.LogWarning("ERROR: Landscape Builder - could not export Object Path points for " + transform.name + " PLEASE REPORT - " + ex.Message);
                    #else
                    // Keep compiler happy
                    if (ex != null) { }
                    #endif
        ...
        /// <summary>
        /// Attempt to find the first landscape in the scene
        /// </summary>
        /// <returns></returns>
        public static LBLandscape FindFirstLandscape()
        {
            #if UNITY_2022_2_OR_NEWER
            return GameObject.FindFirstObjectByType<LBLandscape>();
            #else
            return GameObject.FindObjectOfType<LBLandscape>();
            #endif
        }

        /// <summary>
        /// Get the actual height, in metres, of a normalised height value.
        /// </summary>
        /// <param name="normalisedHeight"></param>
        /// <param name="terrainHeight"></param>
        /// <returns></returns>
        public float GetActualHeight(float normalisedHeight, float terrainHeight)
        {
            return normalisedHeight * terrainHeight + start.y;
        }

        /// <summary>
        /// Return the clamped relative height (between 0.0 and 1.0) of the actual
        /// height on the y-axis in world space.
        /// </summary>
        /// <param name="actualHeight"></param>
        /// <param name="terrainHeight"></param>
        /// <returns></returns>
        public float GetRelativeHeight(float actualHeight, float terrainHeight)
        {
            return Mathf.InverseLerp(start.y, start.y + terrainHeight, actualHeight);
        }
        ...
        /// <returns></returns>
        public static float GetActualHeight (float normalisedHeight, float terrainHeight, Vector3 startPosition)
        {
            return normalisedHeight * terrainHeight + startPosition.y;
        }
        ...
        /// <param name="startPosition"></param>
        /// <returns></returns>
        public static float GetRelativeHeight(float actualHeight, float terrainHeight, Vector3 startPosition)
        {
            return Mathf.InverseLerp(startPosition.y, startPosition.y + terrainHeight, actualHeight);
        }
        ...
        /// NOTE: When updating, also update LandscapeBuilderWindow.GenerateTopography()
        /// </summary>
        /// <param name="showErrors"></param>
        public void ApplyTopography(bool bypassValidation, bool showErrors)
        {
            ...
            // Is there a mask for the landscape? 
            int maskModeInt = 0;
            ...
            if (!InvalidTextures)
            {
                for (int index = 0; index < landscapeTerrains.Length; index++)
                {
                    // If the heightmap is still on the GPU, then it will need to be copied to CPU
                    bool isHeightmapSyncRequired = useGPUTopography && !(maskModeInt != 0 || useThermalErosion || useFinalPassSmoothing);

                    TerrainData _tData = landscapeTerrains[index].terrainData;

                    // If the heightmap is not on the CPU, copy it now
                    if (isHeightmapSyncRequired) { LBLandscapeTerrain.SyncHeightmapToCPU(_tData, _tData.heightmapResolution, true); }

                    landscapeTerrains[index].terrainData = LBLandscapeTerrain.SmoothHeightmapFromLayers(this, _tData, landscapeTerrains[index].transform.position, topographyLayersList);
                }
            }
        ...
        public void ApplyGroups(bool doBackup, bool showProgress)
        {
            ...
            { 
                UnityEditor.EditorUtility.DisplayProgressBar("Backing up " + (isUndoTopographyDisabled ? "" : "heightmap, ") + (isUndoTexturesDisabled ? "" : "tex, ") + (isUndoTreesDisabled ? "" : "trees, ") + (isUndoGrassDisabled ? "" : "and grass ") + "for group placement", "Please Wait", 0.5f);
            }
            #endif

            if (doBackup)
            {
                if (!isUndoTexturesDisabled) { SaveData(LBLandscape.UndoType.Textures); }
                if (!isUndoTreesDisabled) { SaveData(LBLandscape.UndoType.Trees); }
                if (isUndoGrassDisabled) { SaveData(LBLandscape.UndoType.Grass); }
            ...
            if (!LBLandscapeTerrain.PopulateLandscapeWithGroups(lbGroupParm))
            {
                if (doBackup)
                {
                    ...
                    if (!isUndoTexturesDisabled) { RevertToLastSave(LBLandscape.UndoType.Textures); }
                    if (!isUndoTreesDisabled) { RevertToLastSave(LBLandscape.UndoType.Trees); }
                    if (!isUndoGrassDisabled) { RevertToLastSave(LBLandscape.UndoType.Grass); }
                }
            }
                        ...
                        else
                        {
                            ...
                            if (isMeshBuilt)
                            {
                                ...
                                if (meshTransform != null)
                                {
                                    ...
                                    lbWaterParms.lbLighting = LBLighting.FindLBLighting();
        ...
        /// <summary>
        /// A Landscape Extension, by default is a single plane that extends under the whole
        /// landscape. In LB 2.4.0 an option was introduced to allow for a grid of planes around
        /// the landscape.
        /// </summary>
        /// <param name="isEnabled"></param>
        /// <param name="showErrors"></param>
        public void EnableLandscapeExtensionGrid(bool isEnabled, bool showErrors)
        {
            string methodName = "LBLandscape.EnableLandscapeExtensionGrid";
            ...
            else if (useLandscapeExtension)
            {
                ...
                if (extensionTfrm != null)
                {
                    ...
                    DestroyImmediate(extensionTfrm.gameObject);
                }

                if (isEnabled)
                {
                    if (lbLandscapeExtension.gridItemList == null)
                    {
                        lbLandscapeExtension.gridItemList = new List<LBLandscapeExtItem>(8);
                    }

                    int numExtensionGridItems = lbLandscapeExtension.gridItemList.Count;

                    // Ensure there is at least 8 Textures in the list
                    if (numExtensionGridItems < 8)
                    {
                        do
                        {
                            lbLandscapeExtension.gridItemList.Add(new LBLandscapeExtItem());
                            numExtensionGridItems++;
                        } while (numExtensionGridItems < 8);
                    }

                    // Does the single plane already exist?
                    //MeshFilter planeMF;
                    //if (extensionTfrm.TryGetComponent(out planeMF))
                    //{
                    //    DestroyImmediate(extensionTfrm.gameObject);
                    //}

                    GameObject gameObjectExt = new GameObject("LandscapeExtension");

                    if (gameObjectExt != null)
                    {
                        extensionTfrm = gameObjectExt.transform;

                        extensionTfrm.position = new Vector3(start.x, lbLandscapeExtension.heightOffset, start.z);

                        // Centre the grid parent transform under the landscape
                        extensionTfrm.SetParent(landscapeTrfm);
                        //extensionTfrm.localPosition = new Vector3(size.x / 2f, 0f, size.y / 2f);

                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "TopLeft", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "TopMiddle", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "TopRight", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "MiddleLeft", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "MiddleRight", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "BottomLeft", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "BottomMiddle", true);
                        CreateLandscapeExtensionGridPlane(methodName, extensionTfrm, "BottomRight", true);
                    }
                }
                else
                {
                    // Turn off the grid, and create a single plane extension.
                    extensionTfrm = CreateLandscapeExtensionSinglePlane(methodName, landscapeTrfm, extensionTfrm, showErrors);
                }
            }
        }
        ...
        public void EnableLandscapeExtension(bool isEnabled, bool showErrors)
        {
            ...
            else
            {
                ...
                if (isEnabled)
                {
                    ...
                    else if (lbLandscapeExtension.isGridMesh)
                    {
                        EnableLandscapeExtensionGrid(true, showErrors);
                    }
                    else
                    {
                        extensionTfrm = CreateLandscapeExtensionSinglePlane(methodName, landscapeTrfm, extensionTfrm, showErrors);
        ...
        /// <summary>
        /// Create a plane for a grid landscape extension grid slot.
        /// </summary>
        /// <param name="methodName"></param>
        /// <param name="extensionTfrm"></param>
        /// <param name="gridSlot"></param>
        /// <param name="showErrors"></param>
        /// <returns></returns>
        private Transform CreateLandscapeExtensionGridPlane
        (
            string methodName,
            Transform extensionTfrm,
            string gridSlot,
            bool showErrors
        )
        {
            ...
            {
                Transform extensionGridTfrm = gameObjectExtPlane.transform;

                extensionGridTfrm.SetParent(extensionTfrm);
                gameObjectExtPlane.name = gridSlot;
                #if UNITY_2019_2_OR_NEWER
                MeshRenderer mRen;
                if (extensionGridTfrm.TryGetComponent(out mRen))
                #else
                MeshRenderer mRen = extensionGridTfrm.GetComponent<MeshRenderer>();
                if (mRen != null)
                #endif
                {
                    ...
                    {
                        extensionMaterial.name = name + "_" + gridSlot;
                        mRen.sharedMaterial = extensionMaterial;
                    }
                }

                // Calculate the size (a plane is 10x10 units).
                Vector3 extScale = new Vector3(size.x / 10f, 1f, size.y / 10f);

                // Centre the plane under the landscape
                switch (gridSlot)
                {
                    case "TopLeft":
                        extensionGridTfrm.localPosition = new Vector3(-size.x / 2f, 0f, size.y + (size.y / 2f));
                        break;
                    case "TopMiddle":
                        extensionGridTfrm.localPosition = new Vector3(size.x / 2f, 0f, size.y + (size.y / 2f));
                        break;
                    case "TopRight":
                        extensionGridTfrm.localPosition = new Vector3(size.x + size.x / 2f, 0f, size.y + (size.y / 2f));
                        break;
                    case "MiddleLeft":
                        extensionGridTfrm.localPosition = new Vector3(-size.x / 2f, 0f, size.y / 2f);
                        break;
                    case "MiddleRight":
                        extensionGridTfrm.localPosition = new Vector3(size.x + size.x / 2f, 0f, size.y / 2f);
                        break;
                    case "BottomLeft":
                        extensionGridTfrm.localPosition = new Vector3(-size.x / 2f, 0f, -(size.y / 2f));
                        break;
                ...
                extensionGridTfrm.localScale = extScale;

                UpdateLandscapeExtensionTextures(gridSlot, showErrors);

                //UpdateLandscapeExtensionMaterial(showErrors);
                //UpdateLandscapeExtensionHeight(true, showErrors);
            }

            return extensionTfrm;
        }


        /// <summary>
        /// Create a single plane for the Landscape Extension
        /// </summary>
        /// <param name="methodName"></param>
        /// <param name="landscapeTrfm"></param>
        /// <param name="extensionTfrm"></param>
        /// <param name="showErrors"></param>
        /// <returns></returns>
        private Transform CreateLandscapeExtensionSinglePlane(string methodName, Transform landscapeTrfm, Transform extensionTfrm, bool showErrors)
        {
            GameObject gameObjectExtPlane = GameObject.CreatePrimitive(PrimitiveType.Plane);
            if (gameObjectExtPlane != null)
            {
                ...
                #if UNITY_2019_2_OR_NEWER
                MeshRenderer mRen;
                if (extensionTfrm.TryGetComponent(out mRen))
                #else
                MeshRenderer mRen = extensionTfrm.GetComponent<MeshRenderer>();
                if (mRen != null)
                #endif
                {
                    // Disable casting and receiving shadows
        ...
        /// <summary>
        /// Update the main texture of the Landscape Extension grid mesh slot.
        /// Textures go in clockwise order from bottom left.
        /// </summary>
        /// <param name="gridSlot">e.g. BottomLeft, MiddleLeft, TopLeft etc.</param>
        /// <param name="showErrors"></param>
        public void UpdateLandscapeExtensionTextures(string gridSlot, bool showErrors)
        {
            string methodName = "LBLandscape.UpdateLandscapeExtensionTextures";
            Transform landscapeTrfm = this.transform;
            ...
            {
                int numExtensionGridItems = lbLandscapeExtension.gridItemList == null ? 0 : lbLandscapeExtension.gridItemList.Count;

                if (numExtensionGridItems == 8)
                {
                    // Get the parent landscape extension transform
                    Transform extensionTfrm = landscapeTrfm.Find("LandscapeExtension");
                    if (extensionTfrm != null)
                    {
                        // Find the child grid slot extension transform
                        Transform gridSlotTfrm = extensionTfrm.Find(gridSlot);
                        ...
                        {
                            case "TopLeft":
                                gridSlotIndex = 2; break;
                            case "TopMiddle":
                                gridSlotIndex = 3; break;
                            case "TopRight":
                                gridSlotIndex = 4; break;
                            case "MiddleLeft":
                                gridSlotIndex = 1; break;
                            case "MiddleRight":
                                gridSlotIndex = 5; break;
                            case "BottomLeft":
                                gridSlotIndex = 0; break;
                            case "BottomMiddle":
                                gridSlotIndex = 7; break;
                            case "BottomRight":
                                gridSlotIndex = 6; break;
                            default:
                                gridSlotIndex = 0; break;
                        }
                        #if UNITY_2019_2_OR_NEWER
                        MeshRenderer mRen;
                        if (gridSlotTfrm != null && gridSlotTfrm.TryGetComponent(out mRen))
                        #else
                        MeshRenderer mRen = gridSlotTfrm != null ? gridSlotTfrm.GetComponent<MeshRenderer>() : null;
                        if (mRen != null)
                        #endif
                        {
                            Texture2D tex = lbLandscapeExtension.gridItemList[gridSlotIndex].texture;
                            mRen.sharedMaterial.SetTexture("_MainTex", tex);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Gets the detail scatter mode (new in U2022.2).
        /// If and earlier version of Unity or no terrains, defaults to 1
        /// (InstanceCountMode) which is compatible with all versions of LB.
        /// </summary>
        /// <returns></returns>
        public int GetLandscapeDetailScatterMode()
        {
            if (landscapeTerrains != null)
            {
                if (landscapeTerrains.Length > 0)
                {
                    if (landscapeTerrains[0] != null)
                    {
                        ...
                        {
                            #if UNITY_2022_2_OR_NEWER
                            return (int)landscapeTerrains[0].terrainData.detailScatterMode;
                            #else
                            return 1;
                            #endif
                        }
            ...
            else { Debug.LogError("ERROR: LBLandscape GetLandscapeDetailScatterMode - landscapeTerrains not defined"); }
            return 1;
        }
        ...
        /// <summary>
        /// Set the first terrain to DrawInstanced disabled.
        /// See also SetLandscapeTerrainDrawInstancedDelayed(..)
        /// NOTE: Unity will automatically apply this to all terrains in the scene.
        /// </summary>
        public void SetLandscapeTerrainDrawOffInstanced ()
        {
            #if UNITY_2018_3_OR_NEWER
            if (landscapeTerrains != null)
            {
                if (landscapeTerrains.Length > 0)
                {
                    if (landscapeTerrains[0] != null)
                    {
                        landscapeTerrains[0].drawInstanced = false;
                    }
                }
            }
            else { Debug.LogWarning("ERROR: LBLandscape SetLandscapeTerrainDrawInstanced - landscapeTerrains not defined"); }
            #endif
        }

        /// <summary>
        /// Set the first terrain to DrawInstanced enabled.
        /// See also SetLandscapeTerrainDrawInstancedDelayed(..)
        /// NOTE: Unity will automatically apply this to all terrains in the scene.
        /// </summary>
        public void SetLandscapeTerrainDrawOnInstanced ()
        {
            #if UNITY_2018_3_OR_NEWER
            if (landscapeTerrains != null)
            {
                if (landscapeTerrains.Length > 0)
                {
                    if (landscapeTerrains[0] != null)
                    {
                        landscapeTerrains[0].drawInstanced = true;
                    }
                }
            }
            else { Debug.LogWarning("ERROR: LBLandscape SetLandscapeTerrainDrawInstanced - landscapeTerrains not defined"); }
        ...
        /// <summary>
        /// Delay setting terrain Draw Instanced on or off. This is required when creating a new landscape for it to take effect.
        /// NOTE: Unity will change all the terrains in the scene (not just the first terrain of the current landscape).
        /// </summary>
        /// <param name="isEnabled"></param>
        public void SetLandscapeTerrainDrawInstancedDelayed (bool isEnabled)
        {
            Invoke(isEnabled ? "SetLandscapeTerrainDrawOnInstanced" : "SetLandscapeTerrainDrawOffInstanced", 0.2f);
        }
        ...
        /// <summary>
        /// Returns true if the first terrain is enabled
        /// </summary>
        /// <returns></returns>
        public bool TerrainsEnabled()
        {
            SetLandscapeTerrains(false);
            return landscapeTerrains != null && landscapeTerrains.Length > 0 && landscapeTerrains[0].gameObject.activeSelf;
        }
        ...
        /// Currently only used by LBDemoLoader.cs, LBTemplate, and Runtime samples
        ...
        /// <summary>
        /// Will attempt to set the default terrain material on each terrain in the landscape
        /// based on the scriptable render pipline. If U2019.2+ it will also create a new material
        /// for the terrains.
        /// Pre-requisities: landscape.SetLandscapeTerrains(true);
        /// </summary>
        public void SetDefaultTerrainMaterial()
        {
            // Check to see if Universal Render Pipeline is installed in the project
            bool isURP = IsURP(false);

            // Check to see if Light Weight Render Pipeline is installed in this project
            bool isLWRP = !isURP && IsLWRP(false);

            // Check to see if High Definition Render Pipeline is installed in this project
            bool isHDRP = !isURP && !isLWRP && IsHDRP(false);
            ...
            if (isURP || isLWRP || isHDRP || is201920Plus)
            {
                ...
                for (int tIdx = 0; tIdx < numTerrains; tIdx++)
                {
                    terrain = landscapeTerrains[tIdx];
                    SetTerrainMaterial(terrain, tIdx, (tIdx == numTerrains - 1), terrain.terrainData.size.x, ref pixelError, terrainMaterialType);
                }
            }
        }
        ...
        public LBLandscape.TerrainMaterialType GetTerrainMaterialType()
        {
            ...
            else
            {
                ...
                if (terrainCustomMaterial != null)
                {
                    ...
                    // Default RTP shader is "Relief Pack/ReliefTerrain-FirstPass"
                    else if (terrainCustomMaterial.shader.name.Contains("Relief Pack")) { _terrainMaterialType = LBLandscape.TerrainMaterialType.ReliefTerrainPack; }
                    ...
                    // MS v2.x - 3.5 the default shader name is "MicroSplat\terrain name". MS 3.7+ default shader name is "terrain name" so use mat name which is "MicroSplat nn"
                    else if (terrainCustomMaterial.shader.name.Contains("MicroSplat") || terrainCustomMaterial.name.Contains("MicroSplat")) { _terrainMaterialType = LBLandscape.TerrainMaterialType.MicroSplat; }
                    // HDRP 4.9.0+
        ...
        public void RefreshCameraList(List<Camera> cameraList)
        {
            ...
            #if UNITY_2022_2_OR_NEWER
            cameraList.AddRange(GameObject.FindObjectsByType<Camera>(FindObjectsSortMode.None));
            #else
        ...
        public string ScriptLandscapeSettings(string landscapeName, string EndOfLineMarker)
        {
            ...
            sb.Append("\t\t\tnewTerrain.groupingID = " + GetLandscapeTerrainGroupingID().ToString() + ";" + eol);
            sb.Append("\t\t\tnewTerrain.allowAutoConnect = " + GetLandscapeTerrainAutoConnect().ToString().ToLower() + ";" + eol);
            ...
            #if UNITY_2022_2_OR_NEWER
            sb.Append("\t\t\tnewTerrainData.SetDetailScatterMode((DetailScatterMode)" + GetLandscapeDetailScatterMode().ToString() + ");" + eol);
            #endif

            sb.Append("\t\t\tnewTerrainData.wavingGrassSpeed = " + GetLandscapeTerrainGrassWindSpeed().ToString() + "f;" + eol);
            sb.Append("\t\t\tnewTerrainData.wavingGrassAmount = " + GetLandscapeTerrainGrassWindRippleSize().ToString() + "f;" + eol);
            sb.Append("\t\t\tnewTerrainData.wavingGrassStrength = " + GetLandscapeTerrainGrassWindBending().ToString() + "f;" + eol);
            Color grassTintColour = GetLandscapeTerrainGrassWindTint();
            sb.Append("\t\t\tnewTerrainData.wavingGrassTint = new Color(" + grassTintColour.r.ToString() + "f," + grassTintColour.g.ToString() + "f," + grassTintColour.b.ToString() + "f,"  + grassTintColour.a.ToString() + "f);" + eol);
            ...
            sb.Append("\tlandscape.SetDefaultTerrainMaterial();" + eol + eol);
