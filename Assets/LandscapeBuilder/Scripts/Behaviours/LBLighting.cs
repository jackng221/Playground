...
{
    /// <summary>
    /// Works with Built-in Render Pipeline. Not compatible with URP or HDRP.
    ...
    public class LBLighting : MonoBehaviour
    {
        ...
        /// <summary>
        /// Pause or Resume LBLighting time
        /// </summary>
        public bool IsPaused { get { return isPaused; } set { PauseOrResumeTime(value); } }
        ...
        public bool useExternalFog = false;
        ...
        private float currentHailStrength = 0f;
        private float currentSnowStrength = 0f;

        public float CurrentRainStrength { get { return currentRainStrength; } }
        public float CurrentHailStrength { get { return currentHailStrength; } }
        public float CurrentSnowStrength { get { return currentSnowStrength; } }
        ...
        public void Initialise()
        {
            ...
            // In the Unity Lighting editor, AmbientMode.Flat = Color and Trilight = Gradient, Skybox = Skybox
            RenderSettings.ambientMode = GetAmbientMode();
        ...
        /// <summary>
        /// Attempt to find LBLighting in the scene
        /// </summary>
        /// <returns></returns>
        public static LBLighting FindLBLighting()
        {
            #if UNITY_2022_2_OR_NEWER
            return GameObject.FindFirstObjectByType<LBLighting>();
            #else
            return GameObject.FindObjectOfType<LBLighting>();
            #endif
        }
        ...
        {
            #if UNITY_2022_2_OR_NEWER
            Light[] allSceneLights = GameObject.FindObjectsByType<Light>(FindObjectsSortMode.None);
            #else
        ...
        {
            #if UNITY_2022_2_OR_NEWER
            allTerrains = GameObject.FindObjectsByType<Terrain>(FindObjectsSortMode.None);
            #else
        ...
        public void FindReflectionProbes()
        {
            ...
            {
                #if UNITY_2022_2_OR_NEWER
                updatingReflectionProbes = GameObject.FindObjectsByType<ReflectionProbe>(FindObjectsSortMode.None);
                #else
        ...
        /// <summary>
        /// Will pause all LBLighting activiites
        /// </summary>
        public void PauseLighting()
        {
            PauseOrResumeTime(true);
        }

        /// <summary>
        /// Will resume LBLighting activities
        /// </summary>
        public void ResumeLighting()
        {
            PauseOrResumeTime(false);
        }
        ...
        /// <summary>
        /// If useExternalFog is not enabled, update the fog colour and density.
        /// </summary>
        ...
        /// <summary>
        /// Returns the time in hours from the start of the current day
        /// </summary>
        /// <returns>Hours with decimal fraction</returns>
        public float CurrentTimeHour()
        {
            return (CurrentTime() / dayLength) * 24f;
        }
        ...
        public void UpdateLighting(float timeFloat)
        {
            ...
            if (setupMode == SetupMode.Simple)
            {
                ...
                else if (envAmbientSource == EnvAmbientSource.Gradient)
                {
                    // Environment Light mode Gradient
        ...
        public void UpdateWeather(float timeFloat)
        {
            ...
            currentRainStrength = 0f;
            currentHailStrength = 0f;
            currentSnowStrength = 0f;
        ...
        public void RefreshWeatherCameraList(bool updateImageFXComponents, bool showErrors)
        {
            ...
            #if UNITY_2022_2_OR_NEWER
            Camera[] sceneCameras = GameObject.FindObjectsByType<Camera>(FindObjectsSortMode.None);
            #else
        ...
        public void AddParticlesToCamera(Camera camera, PrecipitationType precipitationType, bool showErrors)
        {
            ...
            else
            {
                ...
                {
                    if (cameraParticles[ps].gameObject.CompareTag(_tag)) { alreadyHasWeatherParticles = true; }
                }
        ...
        public void RemoveParticlesFromCamera(Camera camera, PrecipitationType precipitationType, bool showErrors)
        {
            ...
            else
            {
                ...
                for (int ps = 0; ps < cameraParticles.Length; ps++)
                {
                    if (cameraParticles[ps].gameObject.CompareTag(_tag)) { DestroyImmediate(cameraParticles[ps].gameObject); }
                }
            }
        }
        ...
        public void RemoveAllParticlesFromCamera(Camera camera, bool showErrors)
        {
            ...
            else
            {
                ...
                for (int ps = 0; ps < cameraParticles.Length; ps++)
                {
                    ...
                    GameObject _go = cameraParticles[ps].gameObject;
                    if (_go.CompareTag(rainTag) || _go.CompareTag(hailTag) || _go.CompareTag(snowTag)) { DestroyImmediate(_go); }
                }
            }
        }
        ...
        /// <summary>
        /// Get the AmbientMode from the LB Lighting envAmbientSource set in the inspector.
        /// In the Unity Lighting editor, AmbientMode.Flat = Color and Trilight = Gradient, Skybox = Skybox
        /// </summary>
        /// <returns></returns>
        private UnityEngine.Rendering.AmbientMode GetAmbientMode()
        {
             return envAmbientSource == EnvAmbientSource.Skybox ? UnityEngine.Rendering.AmbientMode.Skybox : (envAmbientSource == EnvAmbientSource.Gradient ? UnityEngine.Rendering.AmbientMode.Trilight : UnityEngine.Rendering.AmbientMode.Flat);
        }
        ...
        /// <summary>
        /// Pause or resume the time. This currently is only controlled by exiting Update() early.
        /// If you want to do this in the first frame of game, maybe call UpdateLightingPreview() first. 
        /// </summary>
        /// <param name="isTimePaused"></param>
        private void PauseOrResumeTime(bool isTimePaused)
        {
    ...
    public class LBLightingInspector : Editor
    {
        ...
        private static readonly GUIContent isPausedContent = new GUIContent("Paused", "Is LBLighting paused?");
        ...
        private static readonly GUIContent useExternalFogContent = new GUIContent("Use External Fog", "Use a third party or external product to control fog in the scene. LBLighting will not update fog settings at runtime.");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (lightingScript.showGeneralLightingSettings)
            {
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(isPausedProp, isPausedContent);
                if (EditorGUI.EndChangeCheck() && EditorApplication.isPlaying)
                {
                    lightingScript.IsPaused = isPausedProp.boolValue;
                }
            ...
            if (lightingScript.showAmbientAndFogSettings)
            {
                ...
                if (lightingScript.setupMode == LBLighting.SetupMode.Simple)
                {
                    ...
                    else if (lightingScript.envAmbientSource == LBLighting.EnvAmbientSource.Gradient)
                    {
                        // Gradient Environment lighting (Sky, Ground, Horizon)
                    ...
                    lightingScript.useExternalFog = EditorGUILayout.Toggle(useExternalFogContent, lightingScript.useExternalFog);
