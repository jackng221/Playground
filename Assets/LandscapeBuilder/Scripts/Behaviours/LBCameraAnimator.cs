...
namespace LandscapeBuilder
{
    ...
    public class LBCameraAnimator : MonoBehaviour
    {
        ...
        public void BeginAnimation(bool StartAnimation = true, float StartAtDistance = 0f)
        {
            ...
            lbLighting = LBLighting.FindLBLighting();
            ...
            if (StartAnimation && fadeIn && fadeInDuration > 0f)
            {
                if (lbLighting == null)
                {
                    Debug.LogWarning("LBCameraAnimator.BeginAnimation - cannot find LBLighting in the scene (this is used for fadeIn)");
                }
                else
                {
        ...
        /// <summary>
        /// Reverses the path points. This can let the camera travel along the path in the opposite direction
        /// </summary>
        public void ReversePath()
        {
            if (cameraPath.lbPath != null)
            {
                cameraPath.lbPath.positionList.Reverse();
        ...
        public static LBCameraAnimator CreateCameraAnimator(LBLandscape landscape, string GameObjectName, bool isWakeOnStartEnabled = true)
        {
            ...
            if (animatorObject != null)
            {
                ...
                if (animatorCamera != null)
                {
                    ...
                    LBLighting lbLighting = LBLighting.FindLBLighting();
                    if (lbLighting != null)
                    {
    ...
    public class LBCameraAnimatorInspector : Editor
    {
        ...
        private static GUIContent fadeInContent = new GUIContent("Fade In", "Fade the camera animation in when it first starts (requires LBLighting in the scene)");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (cameraAnimatorScript.animateSpeed)
            {
                ...
                EditorGUILayout.LabelField(new GUIContent("Speed vs Dist Curve", tooltipSpeedDistCurve), GUILayout.Width(EditorGUIUtility.labelWidth - 24f));
                if (GUILayout.Button("R", buttonCompact, GUILayout.MaxWidth(20f)))
                {
                    // Reset curve
                    cameraAnimatorScript.speedTimeCurve = LBCameraAnimator.DefaultDistanceSpeedCurve();
                }

                cameraAnimatorScript.speedTimeCurve = EditorGUILayout.CurveField(cameraAnimatorScript.speedTimeCurve);
