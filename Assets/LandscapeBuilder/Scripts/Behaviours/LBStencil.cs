...
namespace LandscapeBuilder
{
    ...
    public class LBStencil : MonoBehaviour
    {
        ...
        [Range(-0.5f, 0f)] public float meshOffsetY = -0.05f; // To avoid flickering on the terrain
        ...
        /// <summary>
        /// Import a texture (2D Map) into an existing stencil layer
        /// </summary>
        /// <param name="lbStencilLayer"></param>
        /// <param name="texture"></param>
        /// <param name="showErrors"></param>
        /// <returns></returns>
        public bool ImportMapWithOverwrite (LBStencilLayer lbStencilLayer, Texture2D texture, bool showErrors)
        {
            bool isSuccessful = false;

            if (texture == null) { Debug.LogWarning("ERROR: LBStencil.ImportMapWithOverwrite - Texture2D is null - PLEASE REPORT"); }
            else if (texture.width != texture.height) { Debug.LogWarning("ERROR: LBStencil.ImportMapWithOverwrite - non-square textures are not supported"); }
            else if (lbStencilLayer == null) { Debug.LogWarning("ERROR: LBStencil.ImportMapWithOverwrite - is the Stencil Layer is NULL"); }
            else
            {
                isSuccessful = ImportTexture(lbStencilLayer, texture, string.Empty, showErrors);
            }
        ...
        /// <summary>
        /// Import a texture file (2D Map) into an existing Stencil Layer.
        /// Uses the current setting for layerImportMethod (Alpha or Grayscale)
        /// </summary>
        /// <param name="lbStencilLayer"></param>
        /// <param name="folderPath"></param>
        ...
        /// <param name="showErrors"></param>
        /// <returns></returns>
        public bool ImportMapWithOverwrite(LBStencilLayer lbStencilLayer, string folderPath, string fileName, bool showErrors)
        {
            bool isSuccessful = false;
        ...
        public bool ImportTexture(string layerName, Texture2D texture, bool showErrors)
        {
            ...
            {
                isSuccessful = ImportTexture(null, texture, layerName, showErrors);
            }
        ...
        /// <summary>
        /// Import a texture into a Stencil Layer. If the layer is null,
        /// a new layer will be created with the name provided. 
        /// </summary>
        /// <param name="stencilLayer"></param>
        /// <param name="texture"></param>
        /// <param name="showErrors"></param>
        /// <returns></returns>
        public bool ImportTexture (LBStencilLayer lbStencilLayer, Texture2D texture, string layerName, bool showErrors)
        {
            bool isSuccessful = false;
            ...
            else
            {
                ...
                bool isExistingLayer = lbStencilLayer != null;

                if (!isExistingLayer)
                {
                    // Add an empty layer with the matching resolution
                    lbStencilLayer = new LBStencilLayer() { LayerName = layerName };
                }
                ...
                if (lbStencilLayer != null)
                {
                    ...
                    if (lbStencilLayer.compressedTexture != null)
                    {
                        ...
                        int comtexWidth = lbStencilLayer.compressedTexture.width;
                        int comtexHeight = lbStencilLayer.compressedTexture.height;

                        if (isFlipImportTopBottom) { texture = LBTextureOperations.FlipTexture(texture); }

                        // Revalidate the input and output texture sizes
                        if (comtexWidth != texWidth || comtexHeight != texHeight)
                        {
                            if (showErrors) { Debug.LogWarning("ERROR: LBStencil.ImportTexture - the input texture (w:" + texWidth + ", h:" + texHeight + ") does not match the compressed texture size (w:" + comtexWidth + ", h:" + comtexHeight + ")."); }
                        }
                        else
                        {
        ...
        /// <summary>
        /// Using a world space IN the landscape, check to see if it has been painted in the layer provided.
        /// </summary>
        /// <param name="landscape"></param>
        /// <param name="stencilLayer"></param>
        /// <param name="wsPosX"></param>
        /// <param name="wsPosZ"></param>
        /// <param name="normalisedThreshold">Range 0.0 to 1.0</param>
        /// <returns></returns>
        public bool IsPointPaintedInLayer (LBLandscape landscape, LBStencilLayer stencilLayer, float wsPosX, float wsPosZ, float normalisedThreshold)
        {
            ...
            {
                #if UNITY_EDITOR
                Debug.LogWarning("LBStencil.IsPointPaintedInLayer - no landscape provided - it cannot be NULL");
                #endif
            }
            else if (stencilLayer == null)
            {
                #if UNITY_EDITOR
                Debug.LogWarning("LBStencil.IsPointPaintedInLayer - no stencil layer provided - it cannot be NULL");
                #endif
            }
            else
            {
                Vector2 stencilLayerPosN = landscape.NormalisePointFromWorldPos2DFast(new Vector2(wsPosX, wsPosZ));

                if (normalisedThreshold < 0f) { normalisedThreshold = 0f; }
                else if (normalisedThreshold > 1f) { normalisedThreshold = 1f; }

                isPainted = stencilLayer.GetValueAtPoint(stencilLayerPosN.x, stencilLayerPosN.y) >= normalisedThreshold;
            }
    ...
    public class LBStencilInspector : Editor
    {
        ...
        /// <summary>
        /// Used to remember the draw instanced settings on terrains.
        /// In Built-in render pipeline, the projector (brush) isn't
        /// visible if DrawInstanced is enabled.
        /// </summary>
        private bool isDrawInstancedOnTerrain = false;
        ...
        private readonly static GUIContent meshOffsetYPropContent = new GUIContent("Mesh Offset Y", "When adding meshes for use with NavMesh, this can place them slightly below the landscape to avoid flickering.");
        ...
        private readonly static GUIContent importMapIntoLayerContent = new GUIContent("Import and Replace", "Import a square image file into this Stencil Layer (WARNING: Will OVERWRITE existing layer data)");
        ...
        private readonly static GUIContent reverseLayerContent = new GUIContent("N", "Negative Image - reverse or invert the data in the layer");
        ...
        private static readonly string paintingWarning = "There are UNCOMMITTED changes. To save changes, stop painting or stop showing the stencil.";
        private static readonly string uniqueNameWarning = "Please create a unique Stencil Name for the project";
        private static readonly string hdrpWarning = "If a stencil layer in the scene appears as solid colour, try unticking Show Stencil, then showing it again. This is a known problem with HDRP 10+.";
        #endregion
        ...
        private SerializedProperty meshOffsetYProp;
        ...
        public void OnEnable()
        {
            ...
            meshOffsetYProp = serializedObject.FindProperty("meshOffsetY");
            ...
            string editorTextureFolder = LBSetup.editorFolder + "/Textures/";
            ...
            isHDRP = LBLandscape.IsHDRP(false);
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox(uniqueNameWarning, MessageType.Warning);
            }

            if (isHDRP) { EditorGUILayout.HelpBox(hdrpWarning, MessageType.Warning); }
            ...
            if (showStencilSettingsProp.boolValue)
            {
                ...
                EditorGUILayout.PropertyField(meshOffsetYProp, meshOffsetYPropContent);
            }
            #endregion
            ...
            if (showStencilInSceneProp.boolValue && stencilLayerListProp.arraySize > 0)
            {
                ...
                {
                    EditorGUILayout.HelpBox(paintingWarning, MessageType.Warning);
                }
            ...
            if (stencilLayerListProp.isExpanded)
            {
                ...
                if (!lbStencil.brushEnabled)
                {
                    ...
                    if (GUILayout.Button(importMapContent, GUILayout.Width(60f)))
                    {
                        ...
                        else if (layerImportSourceProp.intValue == (int)LBStencil.LayerImportSource.Texture2D)
                        {
                            ...
                            if (LBEditorHelper.GetFilePathFromUser("Import Stencil Layer Data", dataFileFolderRelPath, extensions, true, ref folderPath, ref fileName))
                            {
                                ...
                                string _assetPath = LBEditorHelper.GetAssetPathFromFilePath(filePath);

                                Texture2D tex = AssetDatabase.LoadAssetAtPath(_assetPath, typeof(Texture2D)) as Texture2D;
                                bool texIsReadable = true;

                                if (tex == null) { Debug.LogWarning("LBStencil Import Layer - could not find file at " + _assetPath); }
                                else
                                {
                        ...
                        else if (layerImportSourceProp.intValue == (int)LBStencil.LayerImportSource.Terrains)
                        {
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    Texture2D importHeightMapImage = GetLandscapeHeightmapTexture("Import Stencil Layer from Terrain Heightmap(s)", ref continueToImport);
                    ...
                    if (GUILayout.Button("-", GUILayout.MaxWidth(30f)))
                    {
                        if (stencilLayerListProp.arraySize > 0)
                        {
                            ...
                            removeLayerPos = stencilLayerListProp.arraySize - 1;
                        }
                    }
                ...
                for (layerIndex = 0; layerIndex < stencilLayerListProp.arraySize; layerIndex++)
                {
                    ...
                    if (stencilLayerProp != null)
                    {
                        ...
                        #region Negative image, Move or Insert Layer
                        if (!lbStencil.brushEnabled)
                        {
                            if (GUILayout.Button(reverseLayerContent, buttonCompact, GUILayout.Width(20f)))
                            {
                                GUI.FocusControl(null);

                                serializedObject.ApplyModifiedProperties();
                                lbStencil.stencilLayerList[layerIndex].NegativeLayer();
                                serializedObject.Update();
                                ...
                                else if (showStencilInSceneProp.boolValue)
                                {
                                    //showLayerInSceneToggled = true;
                                    lbStencil.stencilLayerList[layerIndex].UnCompressToUShort();
                                    lbStencil.stencilLayerList[layerIndex].CopyUShortToRenderTexture();
                                }
                            }
                        ...
                        if (showLayerInEditorProp.boolValue)
                        {
                            ...
                            #region Importing Into Existing Layer (Replace)

                            if (GUILayout.Button(importMapIntoLayerContent, GUILayout.Width(120f)))
                            {
                                LBStencilLayer _stencilLayer = lbStencil.stencilLayerList[layerIndex];
                                LBStencil.LayerResolution _origResolution = _stencilLayer.layerResolution;

                                // If this layer is active, turn it off before updating
                                if (_stencilLayer == lbStencil.activeStencilLayer)
                                {
                                    // Is painting current layer?
                                    if (lbStencil.brushEnabled)
                                    {
                                        lbStencil.brushEnabled = false;
                                        EnableBrushPainter(false);
                                ...
                                #region Import from PNG File and Replace existing layer
                                if (layerImportSourceProp.intValue == (int)LBStencil.LayerImportSource.PNG)
                                {
                                    string folderPath = string.Empty, fileName = string.Empty;
                                    string dataFileFolderRelPath = LBEditorHelper.GetAssetsFolder();

                                    if (LBEditorHelper.GetFilePathFromUser("Import and Replace Stencil Layer Data", dataFileFolderRelPath, "png", false, ref folderPath, ref fileName))
                                    {
                                        EditorUtility.DisplayProgressBar("Importing and Replacing Stencil Layer", "Please Wait", 0.2f);

                                        bool isImported = lbStencil.ImportMapWithOverwrite(_stencilLayer, folderPath, fileName, true);
                                        EditorUtility.ClearProgressBar();
                                        if (isImported && showStencilInSceneProp.boolValue)
                                        {
                                            if (_origResolution != _stencilLayer.layerResolution)
                                            {
                                                _stencilLayer.DeallocLayerArray();
                                                _stencilLayer.AllocLayerArray();
                                            }

                                            // Reload the layer
                                            _stencilLayer.UnCompressToUShort();
                                            _stencilLayer.CopyUShortToRenderTexture();
                                ...
                                #region Import from Texture2D in asset folder and replace existing layer
                                else if (layerImportSourceProp.intValue == (int)LBStencil.LayerImportSource.Texture2D)
                                {
                                    string folderPath = string.Empty, fileName = string.Empty;
                                    string dataFileFolderRelPath = LBEditorHelper.GetAssetsFolder();
                                    ...
                                    if (LBEditorHelper.GetFilePathFromUser("Import and Replace Stencil Layer Data", dataFileFolderRelPath, extensions, true, ref folderPath, ref fileName))
                                    {
                                        string filePath = folderPath;
                                        if (!folderPath.EndsWith("/")) { filePath += "/"; }
                                        filePath += fileName;
                                        ...
                                        if (tex == null) { Debug.LogWarning("LBStencil Import and Replace Layer - could not find file at " + _assetPath); }
                                        else
                                        {
                                            try { Color testColour = tex.GetPixel(0, 0); if (testColour.a == 1f) { } }
                                            catch (UnityException e)
                                            ...
                                            if (texIsReadable)
                                            {
                                                //if (lbStencil.ImportTexture(_layerName, tex, true))
                                                if (lbStencil.ImportTexture(_stencilLayer, tex, string.Empty, true))
                                                {
                                                    if (showStencilInSceneProp.boolValue)
                                                    {
                                                        if (_origResolution != _stencilLayer.layerResolution)
                                ...
                                #region Import from existing terrain heightmaps and replace existing layer
                                else if (layerImportSourceProp.intValue == (int)LBStencil.LayerImportSource.Terrains)
                                {
                                    UpdateLandscape(true);
                                    string baseMsg = "ERROR: LBStencil Import Terrain heightmaps into new layer - ";
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            bool continueToImport = true;
                                            Texture2D importHeightMapImage = GetLandscapeHeightmapTexture("Import and Replace Stencil Layer from Terrain Heightmap(s)", ref continueToImport);

                                            // Check to see if the user cancelled
                                            if (continueToImport && importHeightMapImage != null)
                                            {
                                                isImported = lbStencil.ImportMapWithOverwrite(_stencilLayer, importHeightMapImage, true);
                                        ...
                                        if (isImported)
                                        {
                                            // NOTE: Cannot call this within the try-catch as it throus an exception to work.
                                            GUIUtility.ExitGUI();
                                        }
                                    }
                                }
                            ...
                            if (GUILayout.Button(addMeshButtonContent, buttonCompact, GUILayout.MaxWidth(80f)))
                            {
                                ...
                                if (lbStencilLayer.OutlineTexture(outlineTex, Color.blue))
                                {
                                    ...
                                    if (numLBMesh > 0)
                                    {
                                        ...
                                        {
                                            stencilLayerMeshTrfm.localPosition = new Vector3(stencilLayerMeshTrfm.localPosition.x, meshOffsetYProp.floatValue, stencilLayerMeshTrfm.localPosition.z);
                                ...
                                if (lbStencil.landscapeCached.showTiming)
                                {
                                    Debug.Log("Time taken to create stencil layer meshes: " + (Time.realtimeSinceStartup - tStart).ToString("0.000") + " seconds");
                                }
                            }
                            #endregion
                ...
                if (sceneView != null)
                {
                    ...
                    else
                    {
                        ...
                        else if (isShown)
                        {
                            ...
                            // Remember if DrawInstanced was enabled on terrains
                            isDrawInstancedOnTerrain = lbStencil.landscapeCached.GetLandscapeTerrainDrawInstanced();
                            ...
                            else if (!LBLandscape.IsURP(false))
                            {
                                // This is only needed for Build-in RP
                                lbStencil.landscapeCached.SetLandscapeTerrainDrawInstancedDelayed(false);
                            }
                        ...
                        else // Hide Stencil
                        {
                            ...
                            else if (!LBLandscape.IsURP(false) && isDrawInstancedOnTerrain)
                            {
                                // This is only needed for Build-in RP
                                lbStencil.landscapeCached.SetLandscapeTerrainDrawInstancedDelayed(true);
                            }
            ...
            if (lbStencil != null)
            {
                ...
                if (itemList != null)
                {
                    ...
                    for (int item = itemList.Count - 1; item >= 0; item--)
                    {
                        ...
                        {
                            // Remove any (temp) materials created in the assets folder.
                            if (isHDRP)
                            {
                                #if UNITY_2019_2_OR_NEWER
                                MeshRenderer meshRenderer;
                                if (itemList[item].gameObject.TryGetComponent(out meshRenderer))
                                #else
                                MeshRenderer meshRenderer = itemList[item].gameObject.GetComponent<MeshRenderer>();
                                if (meshRenderer != null)
                                #endif
                                {
                                    ...
                                    if (mat != null)
                                    {
                                        LBEditorHelper.DeleteAssetSilently(LBSetup.materialsFolder + "/_" + currentScene, mat.name + ".mat");
                                    }
                                }
                            }
                    ...
                    if (lbStencilLayerItem == null)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                string shaderPath = LBSetup.shadersFolder + "/LBStencilLayer.shader";
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            mat.name = stencilNameProp.stringValue + "_" + lbStencilLayer.LayerName + "_Mat";
                                            mat.SetColor("_MainColour", lbStencilLayer.colourInEditor);
                                            mat.SetTexture("_MainTex", lbStencilLayer.renderTexture);
                                            meshRenderer.sharedMaterial = mat;

                                            // Workaround for HDRP 10+ It is not 100% reliable, sometime have to open and close stencil a couple of times.
                                            // The shader is not compiled correctly when the material is created and renders the solid colour.
                                            // If the material is manually updated in the Inspector it starts working correctly.
                                            if (isHDRP)
                                            {
                                                // Delete any existing material (else it has no effect)
                                                LBEditorHelper.DeleteAssetSilently(LBSetup.materialsFolder + "/_" + currentScene, mat.name + ".mat");
                                                AssetDatabase.CreateAsset(mat, LBSetup.materialsFolder + "/_" + currentScene + "/" + mat.name + ".mat");
                                            }
        ...
        /// <summary>
        /// Get the heightmap of the landscape as a Texture2D.
        /// </summary>
        /// <param name="progressBarTitle"></param>
        /// <param name="continueToImport"></param>
        /// <returns></returns>
        private Texture2D GetLandscapeHeightmapTexture(string progressBarTitle, ref bool continueToImport)
        {
            ...
            for (int index = 0; index < numTerrains; index++)
            {
                ...
                if (EditorUtility.DisplayCancelableProgressBar(progressBarTitle, "Importing heightmap " + (index + 1).ToString() + " of " + numTerrains.ToString() + " terrains",
